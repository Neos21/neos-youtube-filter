...
- [ ] 11. 非表示チャンネルの管理画面を実装する
    ...
    - 検証 : Lint・ビルド成功。フォームが共有 Schema に渡す入力について6件を確認し、片方のみの登録、URL 正規化、不正入力の拒否、識別子補完、空欄の null 化を確認した。ブラウザの依存ライブラリ不足は前タスクから未解消のため、実画面での CRUD・重複/競合エラー表示・入力保持と再試行は未確認。開発者レビュー待ちとし、DB マイグレーションは不要
