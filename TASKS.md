...
- [ ] 10. 非表示動画の管理画面と管理メニューを実装する
    ...
    - 検証 : Lint・ビルド成功。ブラウザ検証は実行環境の `libnspr4.so` 不足で Chromium が起動せず未実施。次回はブラウザで CRUD と各操作後の一覧再取得、動画 URL の正規化、不正入力、API 失敗時の入力保持・再試行、保存成功後の再取得失敗、モバイル表示、401 時のログアウトを確認する。確認完了まで本タスクは未完とする。DB マイグレーションは不要
