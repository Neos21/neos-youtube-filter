...
- [ ] 13. 購読チャンネルの管理画面を実装する
    ...
    - 検証 : Lint・ビルド成功。共有 Schema とメモリ上の SQLite による API 確認で、フォーム入力の正規化・不正入力の拒否、識別子の補完・クリア、CRUD、同じチャンネルのブロック情報が購読情報の更新・削除で変わらないことを確認した。実ブラウザでの操作確認は既知の依存ライブラリ不足により未実施とし、開発者レビュー待ち。DB マイグレーションは不要
