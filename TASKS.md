...
- [ ] 12. 非表示パターンの管理画面を実装する
    ...
    - 検証 : Lint・ビルド・差分チェック成功。共有 Schema への入力18件で文字列/正規表現、空白保持、既定/明示/空フラグ、不正な正規表現・フラグ、PATCH 必須項目を確認した。実ブラウザの操作確認は既知の依存ライブラリ不足により未実施。種別切替、保存失敗後の入力保持と再試行、CRUD と一覧更新は開発者レビュー待ち。DB マイグレーションは不要
