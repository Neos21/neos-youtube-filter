 ...
 * 呼び出し側は成功時の `data` と取得済みレコードを `resolveBlockedChannelUpdate()` に渡し、更新後の内容も検証する
 */
 ...
 * 
 * 呼び出し順は `updateBlockedChannelSchema.safeParse()` → DB の既存レコード取得 → この関数とする
 ...
 * 
 * @param update `updateBlockedChannelSchema.safeParse()` の成功時に得た `data`
 ...
 */
export const resolveBlockedChannelUpdate = (current: CreateBlockedChannel, update: UpdateBlockedChannel): ReturnType<typeof createBlockedChannelSchema.safeParse> => createBlockedChannelSchema.safeParse({
